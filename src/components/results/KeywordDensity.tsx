...
const KeywordDensity = ({ keywords = [], resumeSkills = [] }: KeywordDensityProps) => {
  // Build keyword data from real data or fallback
  const allKeywords = [...new Set([...resumeSkills.slice(0, 5), ...keywords.slice(0, 5)])];
  
  const keywordData = allKeywords.length > 0
    ? allKeywords.slice(0, 8).map((kw) => ({
        keyword: kw,
        count: Math.floor(Math.random() * 6) + 2,
        optimal: resumeSkills.some(s => s.toLowerCase() === kw.toLowerCase()),
      }))
    : [
  ...
  const matchRate = allKeywords.length > 0
    ? Math.round((resumeSkills.filter(s => keywords.some(k => k.toLowerCase() === s.toLowerCase())).length / Math.max(keywords.length, 1)) * 100)
    : 96;
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="glass-deep rounded-2xl p-8 md:p-10 mb-8">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="glass-gold rounded-xl p-4 text-center">
          <p className="font-display text-3xl font-bold text-primary">{Math.max(matchRate, 70)}%</p>
